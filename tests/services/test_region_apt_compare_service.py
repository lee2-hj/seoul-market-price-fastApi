...
포함한다(명세서 5항, Phase 5 세 번째 테스트 항목). 골드 마트에 매칭 row가 없거나 있어도
trade_count == 0이면(과거에는 실버(fact_apt_transactions) 레이어로 거슬러 올라갔으나, 항상
최신 base_date만 사용하도록 이 폴백은 제거되어) 빈 객체({})를 반환한다."""
