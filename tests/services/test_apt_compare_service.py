...
골드 마트(dm_apt_pyeong_price/dm_apt_flr_price) 전체 파티션에서도 매칭 row가 없으면 빈
리스트를 반환한다(실버(fact_apt_transactions) 레이어로 거슬러 올라가는 폴백은 제거됨)."""
...
def test_compare_apartments_returns_empty_list_when_gold_has_no_row(monkeypatch):
    """골드(dm_apt_pyeong_price) 전체 파티션에 매칭 row가 없으면(실버로 거슬러 올라가지 않고)
    빈 리스트를 그대로 반환해야 한다."""
