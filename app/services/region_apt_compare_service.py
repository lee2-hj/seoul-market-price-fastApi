...
def compare_region_apts(
    *,
    ...
    골드 마트에 매칭 row가 없거나 있어도 trade_count == 0이면(최근 90일 거래 0건) 빈 딕셔너리({})를
    반환한다(과거에는 실버(fact_apt_transactions) 레이어까지 온디맨드로 내려가 대체값을 집계했으나,
    항상 최신 base_date 데이터만 사용하도록 이 실버 폴백은 제거되었다)."""
